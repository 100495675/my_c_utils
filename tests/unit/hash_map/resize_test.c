...
Int main(void)
{
  ...
  for (Int i = 0; i < N; ++i)
  {
    ...
    Result_Void_ref_Int r = Hashmap_Int_Int_get(&hm, i);
    assert(Result_Void_ref_Int_is_ok(&r));
    assert(*Result_Void_ref_Int_unwrap(r) == i * 2);
  }

  for (Int i = 0; i < N; i += 3)
  {
    Result_Void_Int popped = Hashmap_Int_Int_pop(&hm, i);
    assert(Result_Void_Int_is_ok(&popped));
  }
