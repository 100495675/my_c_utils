 ...
 */
#define Vector_free(...)         Free(Vector(__VA_ARGS__))
 ...
 */
#define Vector_into_iter(...)    IntoIterator(Vector(__VA_ARGS__))
#define Vector_into_iter_val(...) IntoIterator(Vector(__VA_ARGS__))
#define Vector_iter_mut(...)     IntoIteratorMut(Vector(__VA_ARGS__))
#define Vector_iter_const(...)   IntoIteratorConst(Vector(__VA_ARGS__))
 ...
 */
#define Vector_clone(...)        Clone(Vector(__VA_ARGS__))
 ...
 */
#define iter_Vector_deref(...)   Iterator_deref(iter(Vector(__VA_ARGS__)))
 ...
 */
#define iter_Vector_next(...)    Iterator_next(iter(Vector(__VA_ARGS__)))

/**
 * @brief Destroys and cleans up the iterator.
 * @usage iter_Vector_free(Int)(&it);
 */
#define iter_Vector_free(...)    Iterator_free(iter(Vector(__VA_ARGS__)))
...
#define TEMPLATE_Vector(T) \
    ...
    static inline void TEMPLATE_METHOD(Free, free, ref(Vector(T)))(ref(Vector(T)) *self) { (void)self; } \
    static inline void TEMPLATE_METHOD(Free, free, cref(Vector(T)))(cref(Vector(T)) *self) { (void)self; } \
    static inline ref(Vector(T)) TEMPLATE_METHOD(Clone, clone, ref(Vector(T)))(const ref(Vector(T)) *self) { return *self; } \
    static inline cref(Vector(T)) TEMPLATE_METHOD(Clone, clone, cref(Vector(T)))(const cref(Vector(T)) *self) { return *self; } \
    static inline Vector(T) TEMPLATE_METHOD(Deref, deref, ref(Vector(T)))(ref(Vector(T)) self) { return *self; } \
    static inline Vector(T) TEMPLATE_METHOD(Deref, deref, cref(Vector(T)))(cref(Vector(T)) self) { return *self; } \
    \
    typedef struct \
    { \
        Vector(T) vector; \
    ...
    \
    static inline Result(Void, cref(Char)) Vector_push_back(T)(ref(Vector(T)) self, \
                                                   const T value) \
    { \
    ...
    static inline Result(ref(T), cref(Char)) Vector_at(T)( \
        cref(Vector(T)) self, const Size index) \
    { \
        if (index >= self->size) \
        { \
    ...
    \
    static inline Size Vector_size(T)(cref(Vector(T)) self) \
    { \
        return self->size; \
    } \
    \
    static inline Result(Void, cref(Char)) Vector_set(T)( \
        ref(Vector(T)) self, const Size index, const T value) \
    { \
        if (index >= self->size) \
        { \
            return Result_err(Void, cref(Char))("Index out of bounds"); \
        } \
        Free(T)(&self->data[index]); \
        self->data[index] = value; \
        return Result_ok(Void, cref(Char))((Void){}); \
    } \
    \
    static inline void Vector_clear(T)(ref(Vector(T)) self) \
    { \
    ...
    \
    static inline Result(Void, cref(Char)) Vector_reserve(T)(ref(Vector(T)) self, \
                                                 const Size capacity) \
    { \
    ...
    \
    static inline Result(T, cref(Char)) Vector_pop_back(T)(ref(Vector(T)) self) \
    { \
        if (self->size == 0) \
        { \
    ...
    \
    static inline Result(Void, cref(Char)) Vector_insert_at(T)(ref(Vector(T)) self, \
                                                   Size index, \
                                                    T value) \
    { \
        if (index > self->size) \
    ...
    \
    static inline Result(Void, cref(Char)) Vector_remove_at(T)(ref(Vector(T)) self, Size index) \
    { \
        if (index >= self->size) \
        { \
            return Result_err(Void, cref(Char))("Index out of bounds"); \
    ...
    \
    static inline Result(Void, cref(Char)) Vector_shrink_to_fit(T)(ref(Vector(T)) self) \
    { \
    ...
    \
    static inline void Vector_sort(T)(ref(Vector(T)) self, \
    ...
    \
    static inline void Vector_debug(T)(cref(Vector(T)) self) \
    { \
    ...
    static inline void TEMPLATE_METHOD(Free, free, Vector(T))(ref(Vector(T)) self) \
    { \
        for (Size i = 0; i < self->size; ++i) \
        { \
            Free(T)(&self->data[i]); \
    ...
    static inline Vector(T) TEMPLATE_METHOD(Clone, clone, Vector(T))(cref(Vector(T)) self) \
    { \
        if (!self) \
        { \
        ...
        for (Size i = 0; i < self->size; ++i) \
        { \
            T cloned_value = Clone(T)(&self->data[i]); \
    ...
    static inline iter_Vector(T) TEMPLATE_METHOD(IntoIterator, into_iter, Vector(T))(Vector(T) self) \
    { \
        return (iter_Vector(T)){.vector = self, .index = 0}; \
    } \
    static inline iter_val(Vector(T)) TEMPLATE_METHOD(IntoIterator, into_iter_val, Vector(T))(Vector(T) self) \
    { \
        return (iter_val(Vector(T))){.vector = self, .index = 0}; \
    } \
    static inline iter_mut(Vector(T)) TEMPLATE_METHOD(IntoIteratorMut, into_iter_mut, Vector(T))(ref(Vector(T)) self) \
    { \
        return (iter_mut(Vector(T))){.vector = self, .index = 0}; \
    } \
    static inline iter_const(Vector(T)) TEMPLATE_METHOD(IntoIteratorConst, into_iter_const, Vector(T))(cref(Vector(T)) self) \
    { \
        return (iter_const(Vector(T))){.vector = self, .index = 0}; \
    } \
    static inline Result(ref(T), cref(Char)) TEMPLATE_METHOD(Iterator, next, iter(Vector(T)))(iter(Vector(T)) *self) \
    { \
    ...
    static inline Result(ref(T), cref(Char)) TEMPLATE_METHOD(Iterator, deref, iter(Vector(T)))(const iter(Vector(T)) *self) \
    { \
        if (self->index >= self->vector.size) \
        { \
            return Result_err(ref(T), cref(Char))("Iterator out of bounds"); \
    ...
    static inline void TEMPLATE_METHOD(Iterator, free, iter(Vector(T)))(iter(Vector(T)) *self) \
    { \
        Free(Vector(T))(&self->vector); \
    } \
    static inline Result(T, cref(Char)) TEMPLATE_METHOD(Iterator, next, iter_val(Vector(T)))(iter_val(Vector(T)) *self) \
    { \
        if (self->index >= self->vector.size) \
        { \
    ...
    static inline Result(T, cref(Char)) TEMPLATE_METHOD(Iterator, deref, iter_val(Vector(T)))(const iter_val(Vector(T)) *self) \
    { \
        if (self->index >= self->vector.size) \
        { \
            return Result_err(T, cref(Char))("Iterator out of bounds"); \
    ...
    static inline void TEMPLATE_METHOD(Iterator, free, iter_val(Vector(T)))(iter_val(Vector(T)) *self) \
    { \
        for (Size _i = self->index; _i < self->vector.size; ++_i) \
        { \
            Free(T)(&self->vector.data[_i]); \
        } \
        Free(Vector(T))(&self->vector); \
    } \
    static inline Vector_iter_mut_Result(T) TEMPLATE_METHOD(Iterator, next, iter_mut(Vector(T)))(iter_mut(Vector(T)) *self) \
    { \
        if (self->index >= self->vector->size) \
        { \
    ...
    static inline Vector_iter_mut_Result(T) TEMPLATE_METHOD(Iterator, deref, iter_mut(Vector(T)))(const iter_mut(Vector(T)) *self) \
    { \
        if (self->index >= self->vector->size) \
        { \
            return (Vector_iter_mut_Result(T)){.value = NULL, .is_error = true}; \
    ...
    static inline void TEMPLATE_METHOD(Iterator, free, iter_mut(Vector(T)))(iter_mut(Vector(T)) *self) \
    { \
        (void)self; \
    } \
    static inline Vector_iter_const_Result(T) TEMPLATE_METHOD(Iterator, next, iter_const(Vector(T)))(iter_const(Vector(T)) *self) \
    { \
        if (self->index >= self->vector->size) \
        { \
    ...
    static inline Vector_iter_const_Result(T) TEMPLATE_METHOD(Iterator, deref, iter_const(Vector(T)))(const iter_const(Vector(T)) *self) \
    { \
        if (self->index >= self->vector->size) \
        { \
            return (Vector_iter_const_Result(T)){.value = NULL, .is_error = true}; \
    ...
    static inline void TEMPLATE_METHOD(Iterator, free, iter_const(Vector(T)))(iter_const(Vector(T)) *self) \
    { \
        (void)self; \
    } \
